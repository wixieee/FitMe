...
const Recipe = () => {
  ...
  const { title, description, imageUrl, ingredients, instructions, nutrients, prepTime, calories, youtubeUrl, servings } = recipe;
...
<div className="prep-time">
  <span>
    {nutrients["білки"].match(/\((.*?)\)/)?.[1]}<br />Білки
  </span>
  <span>
    {nutrients["жири"].match(/\((.*?)\)/)?.[1]}<br />Жири
  </span>
  <span>
    {nutrients["вуглеводи"].match(/\((.*?)\)/)?.[1]}<br />Вуглеводи
  </span>
</div>
        ...
        <div className="video-section">
          {!loadVideo ? (
            <div className="video-thumbnail" onClick={() => setLoadVideo(true)}>
              <img src={`https://img.youtube.com/vi/${youtubeId}/hqdefault.jpg`} alt="Watch video" />
            ...
            <iframe
              src={`https://www.youtube.com/embed/${youtubeId}?autoplay=1&controls=1&modestbranding=1&rel=0${isIOS ? "&mute=1" : ""}`}
